...
def _run_inprocess(task_id: str, raw_key: str, raw_bytes: bytes) -> None:
    """인프로세스 실행: 일시 실패는 제한적 재시도(2회) 후 DLQ."""
    for attempt in range(3):
        try:
            process_ingestion(task_id, raw_key, raw_bytes)
            return
...
if settings.task_backend == "celery":
    ...
    def process_ingestion_task(self, task_id: str, raw_key: str, raw_bytes_hex: str):
        raw = bytes.fromhex(raw_bytes_hex)
        try:
            process_ingestion(task_id, raw_key, raw)
        except TransientError as exc:
            try:
                metrics.INGEST_RETRY.inc()
...
def enqueue(task_id: str, raw_key: str, raw_bytes: bytes) -> None:
    ...
    if settings.task_backend == "celery":
        ...
        _executor.submit(_run_inprocess, task_id, raw_key, raw_bytes)
